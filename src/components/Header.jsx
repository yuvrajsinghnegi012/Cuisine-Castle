    ...
    <header className="fixed z-50 w-screen bg-primary p-3 px-4 md:p-6 md:px-16">
      ...
      <div className="hidden md:flex w-full h-full items-center justify-between">
        <Link to={"/"} className="flex items-center gap-2">
          <img src={logo}
            className="w-10 object-cover" alt="logo" />
        ...
        <div className="flex items-center gap-8">
          ...
          <div className="relative">
            <motion.img
              ...
              className="w-10 h-10 min-w-[40px] min-h-[40px] drop-shadow-xl cursor-pointer rounded-full" alt="userprofile"
              onClick={login}
            />
            {
              isMenu && (
                <motion.div
                  ...
                  className="1-40px bg-primary shadow-xl rounded-lg flex flex-col absolute top-12 right-0 px-4 py-2">
                  {/* To Add New Item */}
                  {user && user.email === "yuvrajsinghnegi01@gmail.com" && (
                    <Link to={"/createItem"}>
          ...
          />
          {
            isMenu && (
              <motion.div
                ...
                <p className='m-2 p-2 rounded-md flex justify-center items-center gap-3 cursor-pointer bg-gray-200 hover:bg-gray-300 transition-all duration-100 ease-in-out text-textColor text-base
                '
                  onClick={logout}
                >
                  Logout <MdLogout />
